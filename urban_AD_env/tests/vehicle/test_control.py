...
def test_step():
    ...
    for _ in range(2 * FPS):
        v.step(dt=1/FPS)
    assert v.curr_position[0] == pytest.approx(40)
    assert v.curr_position[1] == pytest.approx(0)
...
def test_lane_change():
    ...
    v = ControlledVehicle(road=road, position=road.network.get_lane((0, 1, 0)).curr_position(0, 0), velocity=20, heading=0)
    v.act('LANE_RIGHT')
    for _ in range(3 * FPS):
        v.act()
        v.step(dt=1/FPS)
    assert v.velocity == pytest.approx(20)
    assert v.curr_position[1] == pytest.approx(StraightLane.DEFAULT_WIDTH, abs=StraightLane.DEFAULT_WIDTH/4)
