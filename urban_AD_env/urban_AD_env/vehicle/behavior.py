...
class IDMVehicle(ControlledVehicle):
    ...
    def __init__(self, 
                 ...
                 timer=None):
        super(IDMVehicle, self).__init__(ini_position, goal_position, route)
        self.enable_lane_change = enable_lane_change
        self.timer = timer or (np.sum(self.curr_position)*np.pi) % self.LANE_CHANGE_DELAY
    ...
    def change_lane_policy(self):
        ...
        for lane_index in self.road.network.side_lanes(self.lane_index):
            ...
            if not self.road.network.get_lane(lane_index).is_reachable_from(self.curr_position):
                continue
...
class LinearVehicle(IDMVehicle):
    ...
    def steering_features(self, target_lane_index):
        ...
        lane_coords = lane.local_coordinates(self.curr_position)
