...
class ControlledVehicle(Vehicle):
    ...
    def __init__(self,
                 ...
                 ini_state,                 
                 route=None):
        super(ControlledVehicle, self).__init__(road, ini_state)        
    ...
    def create_from(cls, vehicle):
        ...
        """
        v = cls(vehicle.road, vehicle.curr_position, heading=vehicle.heading, velocity=vehicle.velocity,
    ...
    def act(self, action=None):
        ...
        if action == "FASTER":
            ...
            if self.road.network.get_lane(target_lane_index).is_reachable_from(self.curr_position):
    ...
    def follow_road(self):
        ...
        """
        if self.road.network.get_lane(self.target_lane_index).after_end(self.curr_position):
    ...
    def steering_control(self, target_lane_index):
        ...
        lane_coords = target_lane.local_coordinates(self.curr_position)
        ...
        steering_angle = np.clip(steering_angle, -vehicle_params.MAX_STEER_ANGLE,  vehicle_params.MAX_STEER_ANGLE)
...
class EGO_Vehicle(ControlledVehicle):
    """
    ...
    def __init__(self,
                 ...
                 goal_state,                 
                 route=None):
        super(EGO_Vehicle, self).__init__(road, ini_state, route)        
        ...
        self.is_on_the_road = True
        
        # Rewards: Just for debugging
        self.collision_reward = 0.0
        self.high_vel_reward = 0.0
        self.reverse_reward = 0.0
        self.lane_change_reward = 0.0
        self.off_road_reward = 0.0
        self.distance_goal_reward = 0.0
        self.reward_wrt_initial_dist_to_goal = 0.0
        self.heading_towards_goal_reward = 0.0
        self.reward = 0.0

    def act(self, action=[None,None]):
        """
            Perform a low-level/Control action.

            You have  to call this method if you want to use a Deterministic policy that outputs directly Steering and Accel

        :param action = [steering,acceleration]
        """
        if np.all(action) == None:
            return

        steering_angle = action[0] *  vehicle_params.MAX_STEER_ANGLE        
        steering_angle = np.clip(steering_angle, - vehicle_params.MAX_STEER_ANGLE,  vehicle_params.MAX_STEER_ANGLE)

        if action[1] >= 0.0:
            acceleration = action[1] * vehicle_params.MAX_ACCEL
        else:
            acceleration = action[1] * -vehicle_params.MAX_DECEL

        action = {'steering': steering_angle,
