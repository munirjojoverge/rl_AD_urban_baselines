...
class IntervalVehicle(LinearVehicle):
    ...
    def observer_step(self, dt, lane_change_model="model"):
        ...
        position_i = self.interval_observer.curr_position
        ...
        if front_observer:
            ...
            lane_psi = self.lane.heading_at(self.lane.local_coordinates(self.curr_position)[0])
            lane_direction = [np.cos(lane_psi), np.sin(lane_psi)]
            diff_i = IntervalVehicle.intervals_diff(front_observer.curr_position, position_i)
        ...
        if lane_change_model == "model":
            ...
            if _id < len(self.road.network.graph[_from][_to]) - 1 \
                    and self.road.network.get_lane((_from, _to, _id + 1)).is_reachable_from(self.curr_position):
                lanes += [(_from, _to, _id + 1)]
        for lane_index in lanes:
            lane = self.road.network.get_lane(lane_index)
            longitudinal_pursuit = lane.local_coordinates(self.curr_position)[0] + self.velocity * self.PURSUIT_TAU
        ...
        self.interval_observer.curr_position[:, 0] += dx_i * dt
        self.interval_observer.curr_position[:, 1] += dy_i * dt
    ...
    def partial_step(self, dt, alpha=0):
        ...
        v_minus.interval_observer.curr_position[1, :] = (1 - alpha) * o.curr_position[0, :] + alpha * o.curr_position[1, :]
        ...
        v_plus.interval_observer.curr_position[0, :] = alpha * o.curr_position[0, :] + (1 - alpha) * o.curr_position[1, :]
        ...
        # 3. Merge the resulting intervals together to x_i(t+dt).
        self.interval_observer.curr_position = np.array([v_minus.interval_observer.curr_position[0], v_plus.interval_observer.curr_position[1]])
    ...
    def check_collision(self, other):
        """
            For robust planning, we assume that EGO_Vehicles collide with the uncertainty set of an IntervalVehicle,
            which corresponds to worst-case outcome.

        :param other: the other vehicle
        """
        if not isinstance(other, EGO_Vehicle):
        ...
        if not utils.point_in_rectangle(other.curr_position,
                                        self.interval_observer.curr_position[0]-self.LENGTH,
                                        self.interval_observer.curr_position[1]+self.LENGTH):
            return
        ...
        projection = np.minimum(np.maximum(other.curr_position, self.interval_observer.curr_position[0]),
                                self.interval_observer.curr_position[1])
...
class VehicleInterval(object):
    def __init__(self, vehicle):
        self.curr_position = np.array([vehicle.curr_position, vehicle.curr_position], dtype=float)
