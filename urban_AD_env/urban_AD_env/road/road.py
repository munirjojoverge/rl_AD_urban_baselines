...
class RoadNetwork(object):
    ...
    def is_inside_network(self, position):
        """
            Tells you if a given world position is inside the Road Network
        ...
        for _from, to_dict in self.graph.items():
            for _to, lanes in to_dict.items():
                for _id, lane in enumerate(lanes):
                    in_road = in_road or lane.on_lane(position)
                    if (in_road):
                        return True
...
class Road(Loggable):
    ...
    def neighbour_vehicles(self, vehicle, lane_index=None):
        ...
        s = self.network.get_lane(lane_index).local_coordinates(vehicle.curr_position)[0]
        s_front = s_rear = None
        v_front = v_rear = None
        for v in self.vehicles:
            if v is not vehicle and self.network.is_connected_road(v.lane_index, lane_index, same_lane=True):
                s_v, lat_v = lane.local_coordinates(v.curr_position)
                if not lane.on_lane(v.curr_position, s_v, lat_v):
                    continue
