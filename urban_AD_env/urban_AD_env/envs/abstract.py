...
from urban_AD_env.envs.finite_mdp import finite_mdp
from urban_AD_env.envs.graphics import EnvViewer
from urban_AD_env.road.lane import AbstractLane
from urban_AD_env.vehicle.behavior import IDMVehicle
...
class AD_UrbanEnv(gym.GoalEnv):
    ...
    """
    PERCEPTION_DISTANCE = 5.0 * EGO_Vehicle.SPEED_MAX
    """
    ...
    def __init__(self, distance_threshold = 5, heading_threshold = 0.08, num_actions = 2, action_space_type='C'):
        ...
        Args:                        
            distance_threshold (float): the threshold (in meters) after which a goal position [x,y] is considered achieved
            heading_threshold (float): the threshold (in radians) after which a goal heading (Theta) is considered achieved
            num_actions (int): Number of actions. For now 2: Steering and Accel/Brake. In the future could be more
            action_space_type (str): 'C' = Continuous or 'D' = Discrete            
        """
        # Seeding
        self.np_random = None
        self.seed()
        ...
        # Action Space
        if action_space_type == 'C':  # Continuous        
            #self.action_space = spaces.Box(np.array([-1,-1]), np.array([+1,+1]), dtype=np.float32)  # steer, gas/brake
            self.action_space = spaces.Box(-1., 1., shape=(num_actions,), dtype=np.float32)
        else: # Discrete
            self.action_space = spaces.Discrete(len(self.ACTIONS))
        
        # for implementation of "Hindsight Experience Replay" HER        
        self.ego_goal_state = np.array([0,0,0,0]) # [x,y,h,v]
        
        # Perception Space 
        perception_space = spaces.Box(shape=(len(self.OBSERVATION_FEATURES)*self.OBSERVATION_VEHICLES,),
                                            low=-1, high=1)

        
        self.observation_space = spaces.Dict(dict(
            desired_goal=spaces.Box(-np.inf, np.inf, shape=self.ego_goal_state.shape, dtype=np.float32),
            achieved_goal=spaces.Box(-np.inf, np.inf, shape=self.ego_goal_state.shape, dtype=np.float32),
            observation=spaces.Box(-np.inf, np.inf, shape=perception_space.shape,dtype=np.float32),
        ))
    ...
    def _get_obs(self):
        ...
        return {
            ...
            'desired_goal': self.ego_goal_state.copy(),
        } 
    ...
    def get_available_actions(self):
        ...
        for l_index in self.road.network.side_lanes(self.vehicle.lane_index):
            if l_index[2] < self.vehicle.lane_index[2] \
                    and self.road.network.get_lane(l_index).is_reachable_from(self.vehicle.curr_position):
    ...
    def change_vehicles(self, vehicle_class_path):
        ...
        :param vehicle_class_path: The path of the class of behavior for other vehicles
                             Example: "urban_AD_env.vehicle.behavior.IDMVehicle"
