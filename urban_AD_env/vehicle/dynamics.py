...
class Vehicle(Loggable):
    ...
    def make_on_lane(cls, road, lane_index, longitudinal, velocity=0):
        ...
        if cls==Obstacle:
           v = cls(road, lane.position(longitudinal, 0)) 
        else:
           v = cls(road, lane.position(longitudinal, 0), lane.heading_at(longitudinal), velocity)
    ...
    def create_random(cls, road, velocity=None, spacing=1):
        ...
        if cls==Obstacle:
            v = cls(road, position=road.network.get_lane((_from, _to, _id)).position(x0, 0))    
        else:
            v = cls(road, position=road.network.get_lane((_from, _to, _id)).position(x0, 0), heading=0, velocity=velocity)
        return v
    ...
    def manouver_duration(self):
        return (datetime.now() - self.manouver_start_time)
    
    def manouver_start_time_reset(self):
        self.manouver_start_time = datetime.timestamp(datetime.now())
    ...
    def to_dict(self, origin_vehicle=None):
        d = {
            ...
            'time_elapsed': (datetime.timestamp(datetime.now()) - self.manouver_start_time)
        }
        if origin_vehicle:
...
class Obstacle(Vehicle):
    ...
    def __init__(self, road, position, heading=0):
        ...
        self.manouver_duration_goal = 0.0 # Manouver goal duration in seconds
